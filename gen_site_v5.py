#!/usr/bin/env python3
"""KB Rewaq v5 — FULL BUSINESS salon site.
Service menu + KWD prices + packages + working WhatsApp booking system.
Target: owner sees HER business online, books HER customers. 10/10 useful.
"""
import os, glob
...
# Each lead: services with KWD price; packages with price
LEADS = [
    {"slug":"midyaf","name_en":"Midyaf Beauty Salon","name_ar":"صالون ميدياف للتجميل",
     ...
     "services":[
        ...
        ("Hair Color & Balayage","صبغ وبلاجاج",25,["Full color","Balayage","Root touch-up"]),
     ...
     "reviews":[("Noura A.","Best salon in Salmiya! My bridal hair was perfect ❤️"),
                ("Sara M.","Always leave looking amazing. So professional."),
                ("Layla K.","Finally a place that understands Arabic styling!")]},
    {"slug":"looknoor","name_en":"Look Noor Ladies Beauty Salon","name_ar":"صالون لوك نور للسيدات",
     ...
     "services":[
        ("Haircut & Blowdry","قص وتصفيف",3,["Haircut","Blowdry"]),
        ("Hair Treatment","علاج الشعر",12,["Protein","Keratin","Extensions"]),
        ("Simple Makeup","مكياج بسيط",10,["Party","Evening","Natural"]),
        ("Bridal Makeup","مكياج عروس",40,["HD bridal","Trial"]),
        ("Glow Facial","فيشل مشرق",5,["Cleanse","Brighten"]),
     ...
     "packages":[
        ("Just 5KD Combo","باقة 5 د.ك",5,"Haircut + Blowdry + Threading"),
     ...
     "reviews":[("Aisha R.","Great prices and lovely staff. My go-to salon now."),
                ("Mariam F.","Haircut + blow dry just 3KD and looks fab!"),
    ...
    {"slug":"monya","name_en":"Monya Ladies Beauty Salon","name_ar":"صالون منى للسيدات",
     ...
     "services":[
        ("Haircut & Styling","قص وتصفيف",3,["Trend cut","Curls","Straighten"]),
        ("Hair Color","صبغ الشعر",20,["Full color","Highlights"]),
        ("Summer Facial","فيشل صيفي",6,["Brightening","Hydration"]),
     ...
     "packages":[
        ("Summer Glow","إشراق الصيف",10,"Brightening Facial + Manicure + Pedicure"),
        ("Monya Special","عرض مونيا",18,"Haircut + Color + Blowdry"),
    ...
    {"slug":"royaljasmine","name_en":"Royal Jasmine Salon","name_ar":"صالون الياسمين الملكي",
     ...
     "services":[
        ("Royal Haircut","قص ملكي",5,["Cut","Style","Blowdry"]),
        ("Balay", "بلاجاج ملكي",28,["Balayage","Ombre","Full color"]),
        ("Anti-Aging Facial","فيشل مكافحة الشيخوخة",18,["Gold","Hydra"]),
        ("Luxury Manicure","مانيكير فاخر",8,["3D nail art","Gel"]),
        ("Pedicure Spa","باديكير سبا",8,["Spa","Scrub"]),
        ("Bridal Makeup","مكياج عروس",50,["HD","Trial","Lashes"]),
        ("Body Scrub","تقشير الجسم",15,["Full body","Moroccan"]),
        ("Massage","مساج",15,["Relax","Hot stone"])],
     "packages":[
        ("Jasmine Glow","إشراق الياسمين",20,"Gold Facial + Hair Spa + Manicure"),
        ("Royal Day","يوم ملكي",35,"Massage + Scrub + Facial"),
        ("Bridal Package","باقة العروس",90,"Bridal Makeup + Hair + Spa")],
    ...
    {"slug":"larene","name_en":"Larene Beauty Salon & Spa","name_ar":"صالون ومنتجع لارين للتجميل",
     ...
     "services":[
        ("Haircut & Color","قص وصبغ",6,["Cut","Color","Style"]),
     ...
     "packages":[
        ("Larene Escape","هروب لارين",15,"Facial + Manicure + Pedicure"),
...
#cur{{position:fixed;top:0;left:0;width:14px;height:14px;border-radius:50%;background:var(--ac);mix-blend-mode:difference;pointer-events:none;z-index:9999;transform:translate(-50%,-50%);transition:width .25s,height .25s}}
#cur.big{{width:54px;height:54px;background:rgba(255,255,255,.25)}}
...
nav.scrolled{{background:rgba(10,10,10,.85);backdrop-filter:blur(16px);padding:14px 6%;border-bottom:1px solid rgba(255,255,255,.06)}}
.logo{{font-family:'Cormorant Garamond',serif;font-size:1.7rem;font-weight:700}}
.nav-links a{{color:#fff;text-decoration:none;margin-left:24px;font-size:.88rem;opacity:.8;transition:.3s}}
...
/* services */
.serv{{padding:110px 6%;background:#0d0d0d}}
...
.scard:hover{{transform:translateY(-8px);border-color:var(--ac);box-shadow:0 20px 50px rgba(0,0,0,.4)}}
...
/* packages */
.pkg{{padding:90px 6%;background:#0a0a0a}}
...
/* booking */
.book{{padding:110px 6%;background:#0d0d0d}}
...
.bookwrap .row{{display:grid;grid-template-columns:1fr 1fr;gap:14px}}
...
.tcard{{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:26px}}
.tcard .stars{{color:#ffd700;font-size:1.1rem;margin-bottom:12px;letter-spacing:2px}}
.tcard p{{font-style:italic;line-height:1.7;opacity:.9;margin-bottom:14px}}
.tcard .who{{font-weight:600;color:var(--ac)}}
/* contact */
.contact{{padding:100px 6%;text-align:center;background:#0d0d0d}}
...
.contact .ar{{color:var(--ac);font-size:1.4rem;margin-bottom:28px}}
.cinfo{{display:flex;gap:36px;justify-content:center;flex-wrap:wrap;margin-bottom:30px}}
...
@media(max-width:860px){{.sgrid,.pgrid,.tgrid{{grid-template-columns:repeat(2,1fr)}}.nav-links{{display:none}}.bookwrap .row{{grid-template-columns:1fr}}}}
</style>
</head>
<body>
<div id="cur"></div>
<script>document.documentElement.classList.remove('no-js');document.body.classList.remove('no-js');</script>
<nav id="nav"><div class="logo">{name_en}</div><div class="nav-links"><a href="#services">Services</a><a href="#packages">Packages</a><a href="#book">Book</a><a href="#reviews">Reviews</a><a href="#contact">Contact</a></div></nav>
...
<section class="book" id="book">
  ...
  <div class="bookwrap reveal">
    <form id="bkf" onsubmit="return sendBook(event)">
      <div class="row">
        <div class="f"><label>Your Name / اسمك</label><input id="bn" required placeholder="Full name"></div>
        <div class="f"><label>Phone / الجوال</label><input id="bp" required placeholder="+965 ......"></div>
      </div>
      <div class="f"><label>Service / الخدمة</label><select id="bs">{SERVICE_OPTS}</select></div>
      <div class="row">
        <div class="f"><label>Date / التاريخ</label><input id="bd" type="date" required></div>
        <div class="f"><label>Time / الوقت</label><input id="bt" type="time" required></div>
      </div>
      <div class="f"><label>Note / ملاحظة</label><textarea id="bm" rows="2" placeholder="Any requests?"></textarea></div>
...
if(window.gsap){{gsap.registerPlugin(ScrollTrigger);gsap.utils.toArray('.reveal').forEach(el=>{{gsap.to(el,{{opacity:1,y:0,duration:1,ease:'power3.out',scrollTrigger:{{trigger:el,start:'top 88%'}},onStart:()=>el.classList.add('show')}});}});}}else{{document.querySelectorAll('.reveal').forEach(el=>el.classList.add('show'));}}
...
const u='https://wa.me/{phone}?text='+encodeURIComponent(msg);window.open(u,'_blank');}}
</script>
</body></html>"""
...
def gen(l):
    ...
    repl={"{name_en}":l["name_en"],"{name_ar}":l["name_ar"],"{phone}":l["phone"],
        ...
        "{HERO}":hero,"{SERVICES}":svcs,"{PACKAGES}":pkgs,"{REVIEWS}":revs,"{SERVICE_OPTS}":opts}
    ...
    print(f'OK {l["name_en"]} | {len(l["services"])} services, {len(l["packages"])} packages')

if __name__=="__main__":
    for l in LEADS: gen(l)
    print("v5 business done")
