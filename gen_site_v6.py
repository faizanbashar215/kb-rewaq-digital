#!/usr/bin/env python3
"""KB Rewaq v6 — SIGNATURE + ALIVE business salon site.
Adds: custom SVG monogram, live OSM map, Open-Now badge, animated counter,
team cards, marquee, horizontal-scroll gallery. Tier A+B from gap analysis.
Tier C (real map pin / team photos / prices) stays owner-supplied.
"""
import os
...
# location: approximate lat/long for area (owner can refine)
LEADS = [
    {"slug":"midyaf","name_en":"Midyaf Beauty Salon","name_ar":"صالون ميدياف للتجميل",
     ...
     "team":[("Heba","Master Stylist","هبة"),("Noura","Bridal Specialist","نورة"),("Sara","Skin Expert","سارة")],
    ...
    {"slug":"looknoor","name_en":"Look Noor Ladies Beauty Salon","name_ar":"صالون لوك نور للسيدات",
     ...
     "lat":29.3358,"lon":48.0801,"clients":192,
     "services":[
     ...
     "team":[("Aisha","Hair Stylist","عائشة"),("Mariam","Makeup Artist","مريم"),("Hessa","Spa Therapist","حصة")],
    ...
    {"slug":"monya","name_en":"Monya Ladies Beauty Salon","name_ar":"صالون منى للسيدات",
     ...
     "lat":29.3342,"lon":48.0788,"clients":176,
     "services":[
     ...
     "team":[("Fatima","Stylist","فاطمة"),("Reem","Nail Artist","ريم"),("Dana","Colorist","دانة")],
    ...
    {"slug":"royaljasmine","name_en":"Royal Jasmine Salon","name_ar":"صالون الياسمين الملكي",
     ...
     "lat":29.3340,"lon":48.0792,"clients":215,
     "services":[
        ("Royal Haircut","قص ملكي",5,["Cut","Style","Blowdry"]),
        ("Royal Balayage","بلاجاج ملكي",28,["Balayage","Ombre","Full color"]),
     ...
     "team":[("Sheikha","Master Stylist","شيخة"),("Noof","Color Specialist","نوف"),("Maryam","Spa Lead","مريم")],
    ...
    {"slug":"larene","name_en":"Larene Beauty Salon & Spa","name_ar":"صالون ومنتجع لارين للتجميل",
     ...
     "lat":29.3318,"lon":48.0335,"clients":203,
     "services":[
     ...
     "team":[("Jawaher","Stylist","جواهر"),("Amna","Therapist","آمنة"),("Salma","Makeup Artist","سلمى")],
...
def monogram(name_en, accent):
    ...
    return f'''<svg viewBox="0 0 100 100" width="46" height="46" aria-hidden="true">
<circle cx="50" cy="50" r="46" fill="none" stroke="{accent}" stroke-width="2"/>
<circle cx="50" cy="50" r="38" fill="none" stroke="{accent}" stroke-width="0.6" opacity="0.5"/>
<text x="50" y="50" text-anchor="middle" dominant-baseline="central" font-family="Cormorant Garamond, serif" font-size="30" font-weight="700" fill="{accent}">{letters}</text>
</svg>'''
...
.mapsec iframe{{width:100%;height:380px;border:0;border-radius:18px;filter:grayscale(1) invert(.9) contrast(1.1)}}
...
<section class="mapsec">
  ...
  <iframe loading="lazy" src="https://www.openstreetmap.org/export/embed.html?bbox={LONL},%20{LATL},%20{LONR},%20{LATR}&layer=mapnik&marker={LAT},%20{LON}" allowfullscreen></iframe>
</section>
...
// open-now (Kuwait UTC+3). Daily 10-21, Fri 14-21
...
def gen(l):
    slug=l["slug"]; pool=COUTURE; hero=pool[0]
    def img(i): return pool[i%len(pool)]
    mono=monogram(l["name_en"], l["accent"])
    # marquee
    marq="".join(f"<span>{s[0]} {s[1]} · {s[2]} KD</span>" for s in l["services"])
    marq=marq+marq  # loop
    # services
    svcs=""; opts=""
    ...
    # horizontal gallery
    hgal="".join(f'<div class="hcard"><img src="{img(i+4)}" alt=""><div class="ov"></div><div class="cap">{l["name_en"]}</div></div>' for i in range(6))
    # team
    team=""
    for (en,role,ar) in l["team"]:
        team+=f'<div class="tcard reveal"><img class="ph" src="{img(0)}" alt=""><h3>{en}</h3><div class="ar">{ar}</div><p>{role}</p></div>'
    # packages
    pkgs=""
    ...
    # map bbox
    la,lo=l["lat"],l["lon"]
    bbox=f"{lo-0.004}%2C{la-0.003}%2C{lo+0.004}%2C{la+0.003}"
    h=TPL
    ...
    print(f'OK {l["name_en"]} | mono+map+stats+team+gallery')

if __name__=="__main__":
    for l in LEADS: gen(l)
    print("v6 signature+alive done")
