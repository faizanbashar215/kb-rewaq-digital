...
LEADS = [
    ...
    {"slug":"looknoor","name_en":"Look Noor Ladies Beauty Salon","name_ar":"صالون لوك نور للسيدات",
     ...
     "packages":[("Just 5KD Combo","باقة 5 د.ك",5,"Haircut + Blowdry + Threading"),
    ...
    {"slug":"monya","name_en":"Monya Ladies Beauty Salon","name_ar":"صالون منى للسيدات",
     ...
     "packages":[("Summer Glow","إشراق الصيف",10,"Brightening Facial + Manicure + Pedicure"),
    ...
    {"slug":"royaljasmine","name_en":"Royal Jasmine Salon","name_ar":"صالون الياسمين الملكي",
     ...
     "services":[("Royal Haircut","قص ملكي",5,["Cut","Style","Blowdry"]),
     ...
     "packages":[("Jasmine Glow","إشراق الياسمين",20,"Gold Facial + Hair Spa + Manicure"),
    ...
    {"slug":"larene","name_en":"Larene Beauty Salon & Spa","name_ar":"صالون ومنتجع لارين للتجميل",
     ...
     "packages":[("Larene Escape","هروب لارين",15,"Facial + Manicure + Pedicure"),
...
@media(max-width:860px){{
  ...
  .ham{{display:flex}}
  .nav-links{{position:fixed;top:64px;right:0;width:min(72vw,260px);height:calc(100vh - 64px);flex-direction:column;align-items:flex-start;gap:6px;padding:24px 22px;background:rgba(10,10,10,.96);backdrop-filter:blur(20px);border-left:1px solid rgba(255,255,255,.08);transform:translateX(110%);transition:transform .35s cubic-bezier(.2,.8,.2,1);overflow-y:auto}}
...
@media(max-width:600px){{
  ...
  .hero p{{font-size:.92rem;padding:0 10px;line-height:1.5}}
  ...
  .serv,.pkg,.team,.testi,.book,.mapsec,.contact{{padding:56px 5%}}
  .sec-t h2{{font-size:clamp(1.7rem,7vw,2.4rem)}}
  .stats{{gap:24px;padding:36px 5%}}
  .stat .num{{font-size:2.4rem}}
  .bookwrap{{padding:22px 16px;border-radius:16px}}
  .bookwrap .row{{grid-template-columns:1fr}}
  .mc-inner{{flex-direction:column;gap:12px;padding:24px 16px;text-align:center}}
  .mc-pin{{font-size:2rem}}
  .cinfo{{flex-direction:column;gap:12px}}
  .wa-float{{bottom:16px;right:16px;padding:11px 15px;font-size:.85rem}}
...
<nav id="nav"><div class="logo">{MONO} {name_en}</div><button class="ham" id="ham" aria-label="Menu"><span></span><span></span><span></span></button><div class="nav-links" id="navLinks"><a href="#services">Services</a><a href="#gallery">Gallery</a><a href="#team">Team</a><a href="#packages">Packages</a><a href="#book">Book</a></div></nav>
...
(function(){{const ham=document.getElementById('ham');const links=document.getElementById('navLinks');if(!ham||!links)return;
